/*
** EPITECH PROJECT, 2024
** instruction_add.c
** File description:
** instruction_add.c
*/

#include "my.h"
#include "op.h"
#include <stdint.h>

void instruction_add(process_t *process, arena_t *arena)
{
    size_t arg1 = process_get_arg_type(process, arena, 0) & op_tab[3].type[0];
    size_t arg2 = process_get_arg_type(process, arena, 1) & op_tab[3].type[1];
    size_t arg3 = process_get_arg_type(process, arena, 2) & op_tab[3].type[2];

    if (!arg1 || !arg2 || !arg3)
        return process_move(process, 1);
    arg1 = process_get_arg_value(process, arena, 0);
    arg2 = process_get_arg_value(process, arena, 1);
    arg3 = process_get_arg_value(process, arena, 2);
    arg1 = process_get_register(process, arg1);
    arg2 = process_get_register(process, arg2);
    arg1 += arg2;
    process_change_register(process, arg3, (uint8_t *)&arg1);
}
